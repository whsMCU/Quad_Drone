...
void cliRadio(cli_args_t *args)
{
	...
	if (args->argc == 2 && args->isStr(0, "show") == true)
	{
		...
		while(cliKeepLoop())
		{
			if (micros()-pre_time >= 10000)
			{
				...
				computeRC(); //2us~10us
				if(radio_ch == ROLL)
					cliPrintf("ROLL RADIO : %d\r\n", RC.rcCommand[ROLL]);
				if(radio_ch == PITCH)
					cliPrintf("PITCH RADIO : %d\r\n", RC.rcCommand[PITCH]);
				if(radio_ch == YAW)
					cliPrintf("YAW RADIO : %d\r\n", RC.rcCommand[YAW]);
				if(radio_ch == THROTTLE)
					cliPrintf("THROTTLE RADIO : %d\r\n", RC.rcCommand[THROTTLE]);
				if(radio_ch == GEAR)
					cliPrintf("GEAR RADIO : %d\r\n", RC.rcCommand[GEAR]);
				if(radio_ch == AUX1)
					cliPrintf("AUX1 RADIO : %d\r\n", RC.rcCommand[AUX1]);
			}
		}
		ret = true;
	}
	...
	if (ret != true)
	{
		cliPrintf("radio show ch[ROL=0, PITCH=1, YAW=2, THROTTLE=3, GEAR=4, AUX1=5]\n");
	}
}
